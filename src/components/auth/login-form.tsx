...
export function LoginForm({
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    trackClick("Navigation", "Clicked signin button", "click");
    ...
    <motion.div variants={container} initial="hidden" animate="show">
      ...
      >
        ...
        >
          ...
          <p className="text-balance text-sm text-slate-400">{t("subtitle")}</p>
        </motion.div>
        <div className="grid gap-6">
          <motion.div variants={item} className="grid gap-2">
            ...
            <Input
              ...
              className="border-slate-700 bg-slate-800/50 text-white focus:border-cyan-700 focus:ring-cyan-900/20"
              {...form.register("email")}
            />
          ...
          <motion.div variants={item} className="grid gap-2">
            <div className="flex items-center">
              ...
              <Link
                ...
                className="ml-auto text-sm text-cyan-400 hover:text-blue-400 transition-colors duration-300"
              >
                {t("forgot_password")}
              </Link>
            </div>
            <div className="relative">
              <Input
                ...
                className="border-slate-700 bg-slate-800/50 text-white focus:border-cyan-700 focus:ring-cyan-900/20 pr-10"
              ...
              <Button
                ...
                variant="ghost"
                size="icon"
                className="absolute right-0 top-0 h-full px-3 py-2 text-slate-400 hover:text-cyan-400"
          ...
          >
            <Button
              type="submit"
              className="w-full bg-gradient-to-r from-blue-600 to-cyan-500 hover:from-blue-700 hover:to-cyan-600 text-white transition-all duration-300 shadow-md hover:shadow-lg hover:shadow-blue-900/30 border-0"
              disabled={isLoading}
            >
              {isLoading ? t("loading") : t("submit")}
            </Button>
          </motion.div>
          <motion.div variants={item} className="text-center text-sm text-slate-400">
            {t("no_account")}{" "}
            <Link
              href="/sign-up"
              className="font-medium text-cyan-400 hover:text-blue-400 transition-colors duration-300"
            >
              {t("register")}
            </Link>
          </motion.div>
